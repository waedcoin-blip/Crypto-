...
export class PositionRepository {
  ...
  public getPositionByMint(mint: string, network?: string, wallet?: string): PositionRecord | undefined {
    const cleanMint = mint.trim();
    return Array.from(this.positions.values()).find(
      p => p.mintAddress === cleanMint && p.state !== 'CLOSED' && (!network || p.network === network) && (!wallet || (p.wallet || 'default') === wallet)
    );
  }

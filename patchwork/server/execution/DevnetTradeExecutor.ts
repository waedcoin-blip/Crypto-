...
export class DevnetTradeExecutor implements TradeExecutor {
  ...
  async buy(params: ExecuteParams): Promise<ExecutionResult> {
    const walletAccount = walletManager.getAccountForExecution('devnet', params.walletAddress || this.defaultWalletIdentity);
    ...
    const decimals = params.tokenDecimals ?? Number(process.env.DEVNET_SHADOW_TOKEN_DECIMALS ?? 6);
    if (!Number.isInteger(decimals) || decimals < 0 || decimals > 18) throw new Error(`INVALID_DEVNET_DECIMALS: ${decimals}`);
    const tokenReceivedRaw = Number(quote.outAmount);
    const solSpent = params.amount / 1e9;
    const tokenQty = tokenReceivedRaw / (10 ** decimals);
  ...
  async getBalance(walletAddress?: string): Promise<number> {
    const account = walletManager.getAccountForExecution('devnet', walletAddress || this.defaultWalletIdentity);

...
router.post('/buy', asyncHandler(async (req, res) => {
  const { network, wallet, mint, amountSol, slippageBps, maxRebuyTimes, tradeOnlyOnce, clientRequestId, label, tpPct, slPct, maxPositions, tokenDecimals } = req.body;
  const response = await tradingEngine.buy({
    ...
    tpPct,
    slPct,
    maxPositions: maxPositions === undefined ? undefined : Number(maxPositions),
    tokenDecimals: tokenDecimals === undefined ? undefined : Number(tokenDecimals),
  });
...
router.post('/sell', asyncHandler(async (req, res) => {
  ...
  const response = await tradingEngine.sell({
    ...
    wallet: wallet || 'default',
    mint,
    amountRaw: amountRaw === undefined ? undefined : Number(amountRaw),
    slippageBps: slippageBps === undefined ? undefined : Number(slippageBps),
    clientRequestId,
    reason,
  });

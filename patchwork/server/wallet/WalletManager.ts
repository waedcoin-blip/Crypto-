...
export class WalletManager {
  ...
  public getAccountForExecution(network: string, wallet: string): WalletAccount {
    const net = network.trim().toLowerCase();
    const requested = wallet.trim();
    const candidates = this.getAllAccounts().filter(a => a.network === net);
    if (!requested || requested === 'default') {
      const fallback = candidates[0];
      if (!fallback) throw new Error(`WALLET_NOT_CONFIGURED: No wallet configured for ${net}`);
      return fallback;
    }
    const byIdentity = this.accounts.get(requested);
    if (byIdentity && byIdentity.network === net) return byIdentity;
    const byPublicKey = candidates.find(a => a.publicKey === requested);
    if (byPublicKey) return byPublicKey;
    const byName = candidates.find(a => a.identity === `${net}:${requested}`);
    if (byName) return byName;
    throw new Error(`WALLET_NOT_CONFIGURED: Wallet ${requested} is not configured for ${net}`);
  }

    ...
    for (const [mint, pos] of Object.entries(positions)) {
      ...
      try {
        ...
        if (rawBalance <= 1000) {
          ...
          const decimals = pos.decimals ?? tokenRegistry.getToken(mint)?.decimals;
          if (!Number.isInteger(decimals) || decimals < 0 || decimals > 18) throw new Error(`UNRESOLVED_TOKEN_DECIMALS: ${mint}`);

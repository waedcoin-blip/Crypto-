...
function App() {
  ...
  const handleManualAddGem = useCallback(async () => {
    ...
    if (isAutoExecutionPending && targetSymbol && targetAddress) {
      ...
      if (hasPosition) {
        ...
        addNotification(`Manual buy removed — ${targetSymbol} must be picked up by the server EntryEngine gate pipeline to be bought.`);
      }
    }
  ...
  useEffect(() => {
    fns.current = { executeAutoSell, executePartialSell, sendTelegramAlert };
  });
  ...
  useEffect(() => {
    ...
    const monitorLoop = async () => {
      ...
      // MONITORING LOOP 2: ENTRIES — REMOVED.
      // Automated paper-buy authorization is exclusively the server's responsibility
      // (EntryEngine -> ServerEntryGate -> HardenedCriteriaEngine -> Gates 1-8).
      // The browser must never authorize entries; it only displays candidate data above.
          ...
          if (needsUpdate && !pendingDiscovery.current.has(trade.tokenAddress)) {
            ...
            fetchTokenSecurityData(trade.tokenAddress).then(security => {
              ...
              setTokenMetrics(m => {
                ...
                // NOTE: client-side auto-buy trigger removed. Automated paper-buy authorization
                // is exclusively the server's responsibility (EntryEngine -> Gates 1-8).
                          ...
                          <td className="p-6 text-center">
                             <div className="flex items-center justify-center gap-2">
                               <button 
                                 onClick={(e) => {
                                   e.stopPropagation();
                                   addNotification('Manual buy removed — paper buys must pass the server EntryEngine gate pipeline.');
                                 }}
                                 ...
                                 title="Manual buy disabled (see gated entry pipeline)"
                               >
                  ...
                  .map((metric: TokenMetric) => (
                    ...
                    >
                      ...
                      <div className="grid grid-cols-2 gap-3 pt-3 border-t border-slate-800/50">
                        ...
                        <button 
                          onClick={() => addNotification('Manual buy removed — paper buys must pass the server EntryEngine gate pipeline.')}

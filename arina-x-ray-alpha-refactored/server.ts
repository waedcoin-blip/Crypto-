...
async function startServer() {
  ...
  app.get("/api/jup/price", async (req, res) => {
    try {
      ...
      // NOTE: `t` is a client-side cache-buster (Date.now()) and is intentionally
      // excluded from the cache key — including it made every request unique and
      // completely defeated the SWR cache, hammering the upstream Jupiter API.
  ...
  app.get("/api/jup/quote", async (req, res) => {
    ...
    try {
      ...
      // excluded from the cache key — including it made every request unique and
      // completely defeated the SWR cache, hammering the upstream Jupiter API.
      const cacheKey = `${inputMint}-${outputMint}-${amount}-${slippageBps}-${baseUrl || 'default'}`;

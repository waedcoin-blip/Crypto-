...
function mapPairToToken(pair: unknown): ScannedToken | null {
  ...
  const address = baseToken?.address || (typeof p.tokenAddress === 'string' ? p.tokenAddress : (typeof p.address === 'string' ? p.address : undefined));
  if (typeof address !== 'string' || !address) return null;

  const p5m = priceChange?.m5 !== undefined && priceChange?.m5 !== null 
    ? safeParseFloat(priceChange.m5) 
    : (priceChange?.h1 !== undefined && priceChange?.h1 !== null ? safeParseFloat(priceChange.h1) / 12 : 0);

  return {
    address,
    symbol: typeof baseToken?.symbol === 'string' ? baseToken.symbol : (typeof p.symbol === 'string' ? p.symbol : 'UNKNOWN'),
    name: typeof baseToken?.name === 'string' ? baseToken.name : (typeof p.name === 'string' ? p.name : 'Unknown'),
    priceUsd: safeParseFloat(p.priceUsd),
    priceChange5m: p5m,
    priceChange1h: priceChange?.h1 !== undefined && priceChange?.h1 !== null ? safeParseFloat(priceChange.h1) : 0,
    priceChange24h: priceChange?.h24 !== undefined && priceChange?.h24 !== null ? safeParseFloat(priceChange.h24) : 0,
    volume24h: safeParseFloat(volume?.h24) || safeParseFloat(p.volume24h),
    liquidityUsd: safeParseFloat(liquidity?.usd) || safeParseFloat(p.liquidityUsd),
    fdv: safeParseFloat(p.fdv),
    marketCap: safeParseFloat(p.marketCap) || safeParseFloat(p.fdv),
...
export class TokenScanner {
  ...
  async scanForNewTokens(): Promise<ScannedToken[]> {
    ...
    try {
      let response = await fetch('/api/dex/tokens/trending', {
        signal: this.abortController.signal,
        headers: { Accept: 'application/json', 'Cache-Control': 'no-cache' },
      });

      if (!response.ok) {
        // Fallback to /api/dex/tokens/discovery-pool
        try {
          const fallbackResp = await fetch('/api/dex/tokens/discovery-pool', {
            signal: this.abortController.signal,
            headers: { Accept: 'application/json', 'Cache-Control': 'no-cache' },
          });
          if (fallbackResp.ok) {
